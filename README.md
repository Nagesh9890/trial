...
def compute_aggregates(df, category_col, amount_col):
    ...
    # Add a column for the total transaction sum for each account
    total_sum_df = df.groupBy("account_number").agg(F.sum(amount_col).alias("total_transaction_sum"))
    agg_df = agg_df.join(total_sum_df, "account_number")
